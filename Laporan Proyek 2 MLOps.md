# Submission 2: Plant Growth Milestone Prediction Cloud Deployment
...
| Solusi machine learning | Untuk menyelesaikan masalah tersebut maka dibuat model klasifikasi untuk menentukan apakah pertumbuhan tanaman baik pada beberapa kondisi tertentu untuk melihat kondisi apa yang paling sesuai. Kemudian agar model dapat digunakan secara umum tidak hanya local saja, maka dilakukan deployment model pada cloud menggunakan railway |
...
| Opsi deployment | Deployment cloud dilakukan dengan menggunakan railway sebagai pengganti Heroku karena Heroku saat ini tidak memberikan layanan gratis |
| Web app | [plant-growth-model](https://mlops-proyek-2-production.up.railway.app/v1/models/plant-growth-model/metadata)|
| Monitoring | Monitoring pada sistem ini dilakukan menggunakan prometheus. Disini dilakukan proses monitoring untuk melihat scrape duration seconds yaitu metrik internal yang digunakan untuk mengukur durasi yang dihabiskan oleh Prometheus untuk meng-scrape (mengambil) metrik dari satu target. Metrik ini memberikan wawasan tentang waktu yang diperlukan untuk mengambil data dari target tertentu, yang dapat membantu dalam memantau kinerja scraping dan mendeteksi masalah potensial. Berdasarkan hasil monitoring durasi terpanjang yang digunakan untuk mengumpulkan metrik adalah 1,6s dan terendahnya adalah 0,117s.|

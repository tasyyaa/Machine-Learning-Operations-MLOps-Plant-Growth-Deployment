...
def input_fn(file_pattern, tf_transform_output, num_epochs=None, batch_size=64):
    ...
    Args:
        file_pattern (str): File pattern for the input data.
        tf_transform_output (TFTransformOutput): Output of the tf.Transform function.
        num_epochs (int, optional): Number of epochs for which data should be repeated.
        batch_size (int, optional): Batch size for training.

    Returns:
        tf.data.Dataset: A dataset containing batches of data.
    """
...
def model_builder(hp):
    ...
    Args:
        hp (HyperParameters): Hyperparameters for building the model.

    Returns:
        tf.keras.Model: The compiled model.
    """
    inputs = {
        transformed_name(key): tf.keras.Input(shape=(1,),
                                              name=transformed_name(key),
                                              dtype=tf.float32)
                                              for key in FEATURE_KEYS
    }
    x = layers.Concatenate()(list(inputs.values()))
    x = layers.Dense(units=hp.Int('units_1', min_value=32, max_value=128, step=32),
                     activation='relu')(x)
    x = layers.Dropout(rate=hp.Float('dropout_1', min_value=0.0, max_value=0.5, step=0.1))(x)
    x = layers.Dense(units=hp.Int('units_2', min_value=16, max_value=64, step=16),
                     activation='relu')(x)
...
def tuner_fn(fn_args: FnArgs) -> TunerFnResult:
    ...
    Args:
        fn_args (FnArgs): Arguments for the tuner function.

    Returns:
        TunerFnResult: A named tuple containing the tuner and fit arguments.
    """
    ...
    early_stopping = EarlyStopping(
        monitor='val_binary_accuracy', mode='max', min_delta=0.001, patience=5, verbose=1)
